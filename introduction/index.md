...
2. Fast mode
    ...
    DCE fast mode is processing the frames by clipping them and keep the valuable parts only. It is reducing the size of the frames so that the time consumption on scanning each frame will be reduced. If the fast mode is enabled, frames will be cut into small pieces before transferred to the application. DCE has four different clipping modes and they will be implemented periodically.
